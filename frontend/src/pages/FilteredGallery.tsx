...
import { MediaFilters, Media } from '../services/media.service';
 ...
 * Shows media matching search filters, grouped by month/year
 * Same layout as SiteGallery but driven by filter query params
 */
export default function FilteredGallery() {
  ...
  const filters: MediaFilters = useMemo(() => {
    ...
    const pilotsParam = searchParams.get('pilots');
  ...
  const { data: media, isLoading, error, refetch } = useMediaSearch(hasFilters ? filters : null);

  // Build human-readable filter summary
  const filterSummary = useMemo(() => {
    const parts: string[] = [];
    if (filters.uploadedBy) parts.push(`Uploaded by: ${filters.uploadedBy}`);
    if (filters.pilots?.length) parts.push(`Pilots: ${filters.pilots.join(', ')}`);
    if (filters.year && filters.month) {
      parts.push(`${MONTH_NAMES[filters.month - 1]} ${filters.year}`);
    } else if (filters.year) {
      parts.push(`Year: ${filters.year}`);
    } else if (filters.month) {
      parts.push(`Month: ${MONTH_NAMES[filters.month - 1]}`);
    }
    return parts.join(' · ');
  }, [filters]);
        ...
        <MobilePageLayout activeTab="media" showBackButton backLabel="Calendar" onBack={() => navigate('/media')}>
          <div className="px-4 py-8">
            <div className="bg-white rounded-xl shadow-elevation-lg p-8">
              <h2 className="text-xl font-bold text-sky-night text-center mb-2">Unable to Load Results</h2>
      ...
      <div className="min-h-screen bg-gray-100">
        ...
        <main className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="bg-white rounded-xl shadow-elevation-lg p-8 animate-scale-in">
            ...
            <h2 className="text-2xl font-display font-bold text-sky-night text-center mb-2">Unable to Load Results</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <button onClick={() => navigate('/media')} className="px-6 py-3 bg-white text-sky-dusk border border-sky-midday font-body font-medium rounded-lg hover:bg-sky-cloud transition-all">
                Back to Calendar
              </button>
            </div>
          </div>
    ...
    return (
      ...
      >
        <div className="px-4 py-4 space-y-4">
          <div>
            <h2 className="text-xl font-bold text-sky-night">Search Results</h2>
            {filterSummary && <p className="text-sm text-sky-dusk">{filterSummary}</p>}
            {media && <p className="text-sm text-sky-dusk">{media.length} {media.length === 1 ? 'item' : 'items'} found</p>}
          </div>
          {!hasFilters ? (
            <div className="bg-white rounded-xl shadow-elevation-lg p-6 text-center">
              <p className="text-sky-dusk mb-4">No filters specified.</p>
              <button onClick={() => navigate('/media')} className="px-6 py-3 bg-sky-morning text-white rounded-lg">Back to Calendar</button>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-elevation-lg p-6 text-center">
              <h3 className="text-lg font-semibold text-sky-night mb-2">No media found</h3>
              <p className="text-sky-dusk mb-4">No media matches the selected filters.</p>
              <button onClick={() => navigate('/media')} className="px-6 py-3 bg-sky-morning text-white rounded-lg">Back to Calendar</button>
            </div>
          )}
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Page header */}
        <div className="mb-6">
          <h1 className="text-3xl font-display font-bold text-sky-night mb-1">Search Results</h1>
          {filterSummary && (
            <p className="text-sky-dusk font-body text-sm">{filterSummary}</p>
          )}
          {media && (
            <p className="text-sky-dusk font-body mt-1">
              {media.length} {media.length === 1 ? 'item' : 'items'} found
              {mediaByMonth.size > 0 && ` across ${mediaByMonth.size} ${mediaByMonth.size === 1 ? 'month' : 'months'}`}
            </p>
          )}
        </div>
        ...
        {!hasFilters ? (
          <div className="bg-white rounded-xl shadow-elevation-lg p-6 sm:p-8">
            <div className="text-center py-12">
              <p className="text-sky-dusk font-body text-lg">No filters specified.</p>
              <button onClick={() => navigate('/media')} className="mt-4 px-6 py-3 bg-sky-morning text-white font-body rounded-lg hover:bg-sky-dusk transition-colors shadow-elevation">
                Back to Calendar
              </button>
            </div>
          </div>
          ...
          <div className="space-y-8">
            {Array.from(mediaByMonth.entries()).map(([monthKey, monthMedia]) => {
              ...
              return (
                <div key={monthKey} className="bg-white rounded-xl shadow-elevation-lg p-6 sm:p-8 animate-fade-in">
                  <div className="mb-6 pb-4 border-b-2 border-sky-midday">
                    <h2 className="text-2xl font-display font-bold text-sky-night">{monthYear}</h2>
          ...
          <div className="bg-white rounded-xl shadow-elevation-lg p-6 sm:p-8">
            <div className="text-center py-12">
              <div className="mb-4">
                <svg className="mx-auto h-16 w-16 text-sky-dusk/50" fill="none" strokeWidth={1.5} stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                </svg>
              </div>
              <h3 className="text-lg font-display font-semibold text-sky-night mb-2">No media found</h3>
              <p className="text-sky-dusk font-body mb-4">No media matches the selected filters.</p>
              <button onClick={() => navigate('/media')} className="px-6 py-3 bg-sky-morning text-white font-body rounded-lg hover:bg-sky-dusk transition-colors shadow-elevation">
                Back to Calendar
              </button>
            </div>
          </div>
